...
serve(async (req) => {
  ...
  try {
    ...
    // Check which content types are missing for today
    ...
    if (needed.length === 0) {
      return new Response(JSON.stringify({ message: "All content already exists for today" }), {
    ...
    for (const type of needed) {
      ...
      if (type === "on_this_day") {
        ...
        prompt = `আজকের জন্য একটি অনুপ্রেরণামূলক উক্তি বাংলায় দাও। text (উক্তিটি) এবং author (লেখকের নাম) দাও।`;
      }
      ...
      const response = await fetch("https://ai.gateway.lovable.dev/v1/chat/completions", {
        ...
        body: JSON.stringify({
          ...
          messages: [
            { role: "system", content: "তুমি একজন বাংলা কন্টেন্ট জেনারেটর। সবসময় বাংলায় উত্তর দাও।" },
      ...
      if (!response.ok) {
        console.error(`AI error for ${type}:`, response.status, await response.text());
        continue;
      }
      ...
      const { error } = await supabase.from("daily_content").upsert({
        ...
        date: today,
        data: parsed,
      }, { onConflict: "content_type,date" });

      if (error) console.error(`Insert error for ${type}:`, error);
